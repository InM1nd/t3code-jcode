...
export function resolveAppModelSelectionForInstance(
  ...
  const preserveUnavailableSelection =
    ...
    !Array.isArray(resolutionOptions) &&
    "preserveUnavailableSelection" in resolutionOptions &&
    resolutionOptions.preserveUnavailableSelection === true;
  ...
  const options = getAppModelOptionsForInstance(
    settings,
    entry,
    preserveUnavailableSelection ? selectedModel : null,
  );
  const resolvedSelection = resolveSelectableModel(entry.driverKind, selectedModel, options);
  if (resolvedSelection) {
    return resolvedSelection;
  }
  if (preserveUnavailableSelection && entry.driverKind === "opencode") {
