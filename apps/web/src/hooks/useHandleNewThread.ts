...
export function useNewThreadHandler() {
  ...
  return useCallback(
    (
      ...
      const withSeedPrompt = (result: { draftId: DraftId; threadId: ThreadId }) => {
        const seedPrompt = options?.seedPrompt?.trim();
        if (seedPrompt) setPrompt(result.draftId, seedPrompt);
        return result;
      };
      ...
      return (async () => {
        ...
        if (
          ...
          return withSeedPrompt({ draftId: racedDraft.draftId, threadId: racedDraft.threadId });
        }
        ...
        return withSeedPrompt({ draftId, threadId });
      })();
    },
