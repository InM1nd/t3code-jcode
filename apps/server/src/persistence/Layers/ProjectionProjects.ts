...
import { ModelSelection, ProjectBoardItem, ProjectScript } from "@t3tools/contracts";
...
const ProjectionProjectDbRow = ProjectionProject.mapFields(
  Struct.assign({
    ...
    boardItems: Schema.fromJsonString(Schema.Array(ProjectBoardItem)),
  }),
);
    ...
    execute: (row) =>
      sql`
        ...
        DO UPDATE SET
          ...
          board_items_json = excluded.board_items_json,
    ...
    execute: ({ projectId }) =>
      sql`
        SELECT
          ...
          board_items_json AS "boardItems",
