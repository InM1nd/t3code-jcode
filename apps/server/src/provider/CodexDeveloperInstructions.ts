...
import { resolveWorkMode } from "./WorkMode.ts";
...
export function buildCodexDeveloperInstructions(
  ...
  const resolvedMode = resolveWorkMode(interactionMode)!;
  const base =
    resolvedMode.nativeInteractionMode === "plan"
      ? codexPlanModeDeveloperInstructions(browserToolsAvailable)
      : codexDefaultModeDeveloperInstructions(browserToolsAvailable);
  const workflowInstruction =
    interactionMode === "debug" || interactionMode === "swarm"
      ? `\n${resolvedMode.instruction}`
      : "";
  return `${base}${workflowInstruction}
